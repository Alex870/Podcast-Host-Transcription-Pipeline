...
Speaker references and corrections are partition-local by default. Glossary paths inherit the global configuration unless you set `preferred_terms_file` or `replacement_map_json` as partition overrides in the workbench. Nothing is shared across spaces accidentally, and no shared speaker profile is assumed.

## Registry and safety

The managed registry is stored in `config/partitions.sqlite3`. Operators do not edit it directly. The launcher can make a timestamped registry backup. The registry rejects overlapping active intake folders, prevents output/state folders from being placed inside intake, and keeps a per-space processing lock and run history. The registry's state directory is a managed boundary; resumable pipeline state and stage artifacts continue to live under the selected output directory.
