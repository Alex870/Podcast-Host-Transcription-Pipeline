...
For separate podcasts, meetings, or other contexts, use option `10` to create or adopt a processing space. Each space has its own intake folder, output folder, checkpoints, corrections, glossary settings, and speaker references. Run a space explicitly after placing new audio in its intake folder; the pipeline does not watch folders in the background.
